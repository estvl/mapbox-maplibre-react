...
const MapNavigation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // On map click, set start/end points
    map.on('click', (e) => {
  ...
  useEffect(() => {
    ...
    // In a real app, you'd calculate progress along the route.
    checkProgressToNextStep();
  }, [userLocation, navigationActive]);

  /**
   * Demo: automatically advance steps after some threshold
   * (In a real app, you'd measure distance to the next step’s endpoint)
   */
  const checkProgressToNextStep = () => {
    if (currentStepIndex >= steps.length - 1) return; // no more steps

    // For demonstration, automatically go to the next step after 10 seconds:
    setTimeout(() => {
